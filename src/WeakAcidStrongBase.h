...
class WeakAcidStrongBase : public TitrationData {
 public:
  WeakAcidStrongBase(const double analyteConcentration,
                     const double valorantConcentration, const double kA,
                     const double V0);
...
WeakAcidStrongBase::WeakAcidStrongBase(const double valorantConcentration,
  ...
  pHeq = 14 + log10(finalConcentrations({{1, anC * v0 / (v0 + vE)}},
                                        {{1, 0}, {1, 0}}, kB)
                        .second[0]);
  std::cout << 14 + log10(finalConcentrations({{1, anC * v0 / (v0 + vE)}},
                                              {{1, 0}, {1, 0}}, kB)
                              .second[0])
            << "\n";
}
...
void WeakAcidStrongBase::setAnalyteConcentration(
    const double analyteConcentration) {
...
void WeakAcidStrongBase::setValorantConcentration(
...
void WeakAcidStrongBase::setEquilibriumConstant(
