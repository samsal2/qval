...
const std::array<double, 15> aY4 =
{1.3e-23,  1.9e-18,  3.3e-14, 2.6e-11,
3.8e-9 , 3.7e-7, 2.3e-5, 5e-4, 5.6e-3,
5.4e-2, 0.36, 0.85, 0.98 , 1 ,1};

class EDTA
{
public:
    EDTA(const double valorantConcentration,
...
EDTA::EDTA(const double valorantConcentration,
           const double metalConcentration,
           const double V0,
           const int pH,
           const double kF)
: anC(metalConcentration), valC(valorantConcentration),
v0(V0), k(aY4.at(pH) * kF), vE(v0 * anC/valC)
{
    pIon0 = -log10(anC);
    pIonEq = -log10(finalConcentrations({{1,0}, {1,0}}, {{1, anC * v0/(v0 + vE)}}, k).first[0]);
}
