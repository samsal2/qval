...
double helperDerivative(const std::array<double, 3>& concentrations,
                        ...
                        const double x)
{
    double derivative0 = helperIndividualDerivative(-x, concentrations[0], coefficients[0]),
    derivative1 = helperIndividualDerivative(x, concentrations[1], coefficients[1]),
    derivative2 = helperIndividualDerivative(x, concentrations[2], coefficients[2]),
    ...
    return (value0 * (derivative1 * value2 + derivative2 * value1) - derivative0 * value1 * value2)/(value0 * value0);
}
...
std::array<double, 3> finalConcentrations(const std::array<double, 3>& concentrations,
                                          const std::array<int, 3>& coefficients,
                                          const double k,
                                          const std::size_t&& iterations = 30)
{
    ...
    for(std::size_t i = 0; i < iterations; i++)
        x = x - f(concentrations, coefficients, k, x)/helperDerivative(concentrations, coefficients, x);
    
    //std::cout << "newton-rapson result: " << x << "\n";
    
    return std::array<double, 3>{concentrations[0] - x, concentrations[1] + x, concentrations[2] + x};
}
