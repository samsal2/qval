...
std::pair<std::vector<double>, std::vector<double>> finalConcentrations(
    const std::vector<std::pair<int, double>>& reactData,
    const std::vector<std::pair<int, double>>& prodData, const double k,
  ...
  for (const std::pair<int, double>& data : reactData)
    denominatorExpansion *=
        Polynomial(data.second, -double(data.first), data.first);
