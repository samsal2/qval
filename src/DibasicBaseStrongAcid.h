...
DibasicBaseStrongAcid::DibasicBaseStrongAcid(const double valorantConcentration,
                                             ...
                                             const double V0, const double kA,
                                             const double kA2)
    : TitrationData(valorantConcentration, analyteConcentration, V0, kA),
      ...
      vE2(vE * 2) {
  pH0 = 14 +
        log10(finalConcentrations({{1, anC}}, {{1, 0}, {1, 0}}, kB1).second[0]);
  pHeq = [&]() {
    double F = anC * (v0 / (v0 + vE));
    return -log10(sqrt((k * k2 * F + k * 1e-14) / (k2 + F)));
  }();
  pHeq2 = -log10(
      finalConcentrations({{1, anC * v0 / (v0 + vE2)}}, {{1, 0}, {1, 0}}, k2)
          .second[0]);
}
...
void DibasicBaseStrongAcid::setStartingVolume(const double startingVolume) {
  ...
  pHeq2 = -log10(finalConcentrations({{1, anC * v0 / (v0 + vE2)}}, {{1, 0}}, k2)
                     .second[0]);
}

void DibasicBaseStrongAcid::setAnalyteConcentration(
    const double analyteConcentration) {
...
void DibasicBaseStrongAcid::setValorantConcentration(
...
void DibasicBaseStrongAcid::setEquilibriumConstant(
...
void DibasicBaseStrongAcid::setEquilibriumConstant2(
    const double equilibriumConstant) {
