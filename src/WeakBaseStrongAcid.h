...
{
    pH0 = 14 + log10(finalConcentrations({{1, anC}}, {{1,0}, {1,0}}, kB).second[0]);
    pHeq = -log10(finalConcentrations({{1, anC * v0/(v0 + vE)}}, {{1, 0}, {1,0}}, k).second[0]);
}
...
void WeakBaseStrongAcid::setAnalyteConcentration(const double analyteConcentration)
{
...
void WeakBaseStrongAcid::setValorantConcentration(const double valorantConcentration)
{
...
void WeakBaseStrongAcid::setEquilibriumConstant(const double equilibriumConstant)
{
